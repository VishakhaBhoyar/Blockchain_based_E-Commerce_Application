...
if($obj->StoreProduct($pname,$price,$desc,$mfd,$dis,$city,$qty,$main_cate,$sub_cate,$unit,$status)==1)
{
  ...
  $stmt = $obj1->db->prepare("SELECT * FROM ep_user WHERE TYPE_BUSINESS='$typeb'"); // $obj->db is refer to the connection db is the public variable define in ADB_Fuctions file
			$stmt->execute();
                ...
                <?php while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {?>
                                                     ...
                                                     <option value="<?php echo $row['NAME_BUSINESS']; ?>"><?php echo $row['NAME_BUSINESS']; ?></option>
                                                      <?php }?> 
                                                </select>
                                            </div>
                                        </div>
